# -*- coding: utf-8 -*-
"""
Created on Thu Apr 28 16:09:41 2022
...
for i in range (5, 0, -1):
    ...
    if i == 5:
        total_trainset = DatasetSimplifiedVersion(csv_file = csv_file, root_dir = train_vars_dict["root_dir_train" + str (i)], transform = transform_train)
    else:
        total_trainset += DatasetSimplifiedVersion(csv_file = csv_file, root_dir = train_vars_dict["root_dir_train" + str (i)], transform = transform_train)
        
    train_vars_dict["trainloader" + str (i)] = torch.utils.data.DataLoader(total_trainset, batch_size = batch_size, shuffle = True, num_workers = num_workers)
...
while not isLast:
    ...
    for i, data in enumerate(train_vars_dict["trainloader" + str (6 - numSubsets)], 0):
    ...
    if acc >= accMaxTest:
        accMaxTest = acc
        torch.save(model, 'babyStep')
        epochMax = epoch
        

bestModel = torch.load('babyStep')
